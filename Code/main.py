...
def render(render_type, eye_cascade, face_cascade):
    ...
    if render_type == Render_Type.CAMERA_VIDEO:
        ...
        while True:
            ...
            exec_time_ms = (time.perf_counter() - start_time) * 1000  # Convert to milliseconds
            mem_usage_mb = process.memory_info().rss / (1024 * 1024)  # Convert bytes to MB
            cpu_usage = process.cpu_percent()  # CPU % used by this script since last call

            # Draw stats on the screen
            cv.putText(frame, f"Time/Frame: {exec_time_ms:.1f} ms", (0, 120), cv.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 1)
            cv.putText(frame, f"Memory: {mem_usage_mb:.1f} MB", (0, 135), cv.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 1)
            cv.putText(frame, f"CPU Usage: {cpu_usage:.1f}% ({total_cores} Cores)", (0, 150), cv.FONT_HERSHEY_PLAIN, 1,
                       (0, 255, 0), 1)
            # ---------------------------------

            session_results.add_data(cpu_usage, mem_usage_mb, exec_time_ms)
            ...
            if digit == 105:
                ...
                session_results.print_averages("Camera Video")
                return Render_Type.IMAGE, False
            # letra G
            elif digit == 103:
        ...
        while True:
            ...
            filtered_image = filtering.add_filter(image, is_negative, has_color_filter, color_filter)
            ...
            # Draw stats on the screen
            cv.putText(filtered_image, f"Time/Frame: {exec_time_ms:.1f} ms", (0, 120), cv.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 1)
            cv.putText(filtered_image, f"Memory: {mem_usage_mb:.1f} MB", (0, 135), cv.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 1)
            cv.putText(filtered_image, f"CPU Usage: {cpu_usage:.1f}% ({total_cores} Cores)", (0, 150), cv.FONT_HERSHEY_PLAIN, 1,
                       (0, 255, 0), 1)
            # ---------------------------------
            ...
            if digit == 118:
                cv.destroyAllWindows()
                session_results.print_averages("Image")
